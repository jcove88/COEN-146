/* Jillian Coveney
 * Thursday 5:15 
 * Lab5, step 1: UDP server with packets that have checksum, sequence number, acknowledgement number
 */
...
int main(int argc, char *argv[]) {
    ...
    if((bind(sockfd,(struct sockaddr*) &servAddr, sizeof(struct sockaddr))) < 0){
        perror("failure to bind server socket to endppoint\n");
        exit(1);
    }
    ...
    do {
        ...
        write(fp, packet.data, packet.header.len); //(packet.data)
