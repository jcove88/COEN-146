/* Jillian Coveney
 * Thursday 5:15
 * Lab7: This program computes and demonstrates Link State routing algorithm
 */ 
...
void * receive_info (void *arg)
{
	...
	int n0, n1, n2;
	while(1){
		recvfrom(sock, new, sizeof(int) * 3, 0, NULL, NULL);
		n0 = ntohl(new[0]);
		n1 = ntohl(new[1]);
		n2 = ntohl(new[2]);
...
void * run_link_state (void *arg)
{
	...
	while (1)
	{
		...
		printf("Link state is running.\n");
		for (i = 1; i < N; i++)
		{
			// find closest node
			min = INFINITE;
			for(j = 0; j < N; j++){
				if(taken[j] != 1 && min > distances[j]){ 
					min = distances[j];
					spot = j;
				}
			}
